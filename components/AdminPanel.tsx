...
// ... (ManageOpenEnrollments, ManageOngoingCourses, etc. remain largely the same)
...
const ManageVideo: React.FC<Pick<AdminPanelProps, 'videoUrls' | 'setVideoUrls'>> = ({ videoUrls, setVideoUrls }) => {
    ...
    const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            setVideoUrls(prev => [...prev, objectUrl]);
            addToast('Upload do vídeo concluído!', 'success');
        }
    ...
    const handleDelete = () => {
        if (deletingIndex !== null) {
            ...
            // Revoke object URL to free up memory if it's a blob
        ...
        <>
            <div className="bg-black/20 p-4 rounded-lg">
                ...
                <form onSubmit={handleAddUrl} className="flex flex-col sm:flex-row gap-2 mb-2">
                    <input type="url" placeholder="Cole a URL do vídeo (.mp4, .webm)" value={newUrl} onChange={e => setNewUrl(e.target.value)} className="flex-grow p-2 rounded bg-gray-700 focus:outline-none focus:ring-2 focus:ring-[#f58220]" />
                ...
                <div className="text-center">
                    ...
                    <p className="text-xs text-gray-400 mt-2">
                        Aviso: Vídeos carregados por upload são temporários e serão perdidos ao recarregar a página.
                    </p>
                </div>
                        ...
                        <ul className="space-y-2 pr-2">
                            {videoUrls.map((url, index) => (
                                <li key={index} className="bg-gray-700 p-2 rounded flex justify-between items-center gap-4">
                                    <span className="text-sm break-all">{url.startsWith('blob:') ? `(Upload Local) Video ${index + 1}` : url}</span>
            ...
            <ConfirmDeleteModal isOpen={isConfirmOpen} onClose={() => setIsConfirmOpen(false)} onConfirm={handleDelete} itemName={urlToDelete.startsWith('blob:') ? `Video ${deletingIndex !== null ? deletingIndex + 1 : ''}` : urlToDelete}/>
        </>
    );
}
...
const ManageUsers: React.FC<Pick<AdminPanelProps, 'users' | 'setUsers' | 'currentUser'>> = ({ users, setUsers, currentUser }) => {
    ...
    const handleSave = (e: React.FormEvent) => {
        ...
        if ('id' in editingUser) {
            // Cannot edit users in this simplified version, only add/delete
        } else {
...
const ManageSettings: React.FC<Pick<AdminPanelProps, 'settings' | 'setSettings'>> = ({ settings, setSettings }) => {
    ...
    const handleLogoUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
        const file = event.target.files?.[0];
        if (file) {
             // Limit file size to 500KB to prevent storage issues
